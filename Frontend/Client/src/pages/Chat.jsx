    ...
    <div className="bg-[url('/assets/blueBackground.jpg')] bg-cover bg-center relative flex justify-center items-center h-screen">
      <div className="absolute inset-0 bg-black/70 text-[#D8F4F6] flex flex-col font-audiowide">
        ...
        <div className='flex-grow flex justify-center items-center mb-8 '>
          <div className='md:h-[80vh] max-w-2xl bg-gradient-to-br from-[#1e3c72] to-[#2a5298] md:w-[85vw]   p-5 rounded-2xl shadow-2xl flex flex-col justify-between border border-white/20'>
            ...
            <div className="flex-1 overflow-y-auto mb-6 rounded-lg bg-white/10 p-4">
              {messageArrays && (
                messageArrays.map((msg, index) => (
                  <div className='font-thin text-white' key={index}>
                    {msg.sender && (
                      <div className="mb-2 p-3 rounded-lg text-sm bg-white/50 text-white/80  w-fit max-w-[70%] break-words">{msg.sender}
                    </div>
                    )}
                    {msg.ai && ( <div className="mb-2 p-3 rounded-lg text-sm bg-white/20 text-white w-fit max-w-[80%] break-words ">
